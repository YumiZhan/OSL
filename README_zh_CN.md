...
- `argument`和`agm`：一般用作函数参数的数字类型`const double&`的别名
...
以动态分配一维数组为基础，`vector` 类对象可用于表示向量、有序数列或者行矩阵。比 `std::array` 略胜一筹的是，用户可以随时改变它的长度又无需操心于内存的分配和释放。支持以 `double` 和 Number 库中的类作为元素类型。
