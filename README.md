...
- `rational(unsigned code, c_str discription)`：用异常码和一个字符串来构造对象
- `overview()`：返回对应异常码的概述
...
- `simplify_fraction()`：将过大或过小的指数同时减小或增大一个相同的值，从而限制分子和分母的取值范围，使其不至于过大或过小；当遇到分子和分母同时为无穷大或含有非数时，会抛出异常
...
- `*`：向量点乘
- `<<`：将向量以 `x\ty\tz...` 的形式插入到流中
- `>>`：从流中读取向量，用空格、制表符或换行来分开各元素

##### (六) 实用工具

- `print(c_str separator)`：将向量流输出到控制台窗口中，`separator` 为分隔符
...
- `[](int row)`：返回第 `row` 行的 `vector` 对象备份
- `<<`：将矩阵插入到流中
- `>>`：从流中读取矩阵，用空格、制表符或换行来分开各元素

##### 6. 实用工具

- `print(c_str separator)`：将矩阵流输出到控制台窗口中，`separator` 为列分隔符
